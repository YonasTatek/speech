...
class RecAUD:
    ...
    def __init__(self, chunk=3024, frmat=pyaudio.paInt16, channels=2, rate=44100, py=pyaudio.PyAudio()):
        ...
        self.strt_rec = tkinter.Button(self.buttons, width=10, padx=10, pady=5, text='ጀምር', command=lambda: self.start_record())
        self.strt_rec.grid(row=0, column=0, padx=50, pady=5)
        self.stop_rec = tkinter.Button(self.buttons, width=10, padx=10, pady=5, text='ተው', command=lambda: self.stop())
